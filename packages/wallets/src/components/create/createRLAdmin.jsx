...
import { AlertDialog, skynetToSynt } from '@skynet/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      skynetper_input.value === '' ||
      Number(skynetper_input.value) === 0 ||
      !Number(skynetper_input.value) ||
      isNaN(Number(skynetper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const skynetper = skynetToSynt(skynetper_input.value);
    ...
    // var fee = skynetToSynt(fee_input.value);
