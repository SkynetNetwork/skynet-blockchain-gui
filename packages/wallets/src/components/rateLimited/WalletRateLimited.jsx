...
import { AlertDialog, Card, Flex, skynetToSynt, syntToSkynetLocaleString } from '@skynet/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hexnteck = /[\da-f]+$/gi;

    if (!hexnteck.test(ip_val) || ip_val.value === '') {
    ...
    const skynetper_input = ip_parsed.limit;
    ...
    const skynetper_value = Number.parseInt(Number(skynetper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (skynet per interval):{' '}
                  {syntToSkynetLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {syntToSkynetLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = skynetToSynt(amount_input.value);
    const fee = skynetToSynt(fee_input.value);
