...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, TableControlled, toBech32m, useCurrencyCode, syntToSkynetLocaleString, syntToCATLocaleString } from '@skynet/core';
import styled from 'styled-components';
import type { Row } from '@skynet/core';
import { WalletType, TransactionType } from '@skynet/api';
...
const getCols = (type: WalletType) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            {type === WalletType.CAT
            ? syntToCATLocaleString(row.amount)
            : syntToSkynetLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{syntToSkynetLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
