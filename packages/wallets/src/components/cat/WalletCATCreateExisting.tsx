...
import { AlertDialog, Fee, Back, ButtonLoading, Card, Flex, Form, TextField } from '@skynet/core';
import { Box, Grid } from '@material-ui/core';
import { useDispatch } from 'react-redux';
import { useForm } from 'react-hook-form';
import { useNavigate } from 'react-router';
import { create_cc_for_colour_action } from '../../../modules/message';
import { skynetToSynt } from '@skynet/core';
...
export default function WalletCATCreateExisting() {
  ...
  async function handleSubmit(values: CreateExistingCATWalletData) {
    try {
      ...
      const feeSynts = skynetToSynt(fee || '0');

      const response = await dispatch(create_cc_for_colour_action(name, feeSynts));
      ...
      <Flex flexDirection="column" gap={3}>
        <Back variant="h5">
          {asteroid 
            ...
            : <Trans>Create Skynet Asset Token Wallet from Existing TAIL</Trans>}
