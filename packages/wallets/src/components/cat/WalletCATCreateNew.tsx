...
import { skynetToSynt, AlertDialog, Amount, Fee, Back, ButtonLoading, Card, Flex, Form } from '@skynet/core';
  ...
  async function handleSubmit(values: CreateCATWalletData) {
    try {
      ...
      */
      ...
      const amountSynts = skynetToSynt(amount || '0');
      const feeSynts = skynetToSynt(fee || '0');


      /*
      const response = await dispatch(create_cc_action(amountSynts, feeSynts));
  ...
  return (
    <Form methods={methods} onSubmit={handleSubmit}>
      <Flex flexDirection="column" gap={3}>
        <Back variant="h5">
          <Trans>Create New Skynet Asset Token Wallet</Trans>
        </Back>
        <Card>
