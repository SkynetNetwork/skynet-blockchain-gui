...
} from '@skynet/api-react';
import { SyncingStatus } from '@skynet/api';
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send skynet to coloured address. Please enter a skynet address.`);
    }
    ...
    if (address.slice(0, 12) === 'skynet_addr://') {
    ...
    const amountValue = catToSynt(amount);
    const feeValue = skynetToSynt(fee);
