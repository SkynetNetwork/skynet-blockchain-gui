...
import { AlertDialog, Card, Flex } from '@skynet/core';
...
import {
  synt_to_colouredcoin_string,
  colouredcoin_to_synt,
} from '../../../util/skynet';
      ...
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {synt_to_colouredcoin_string(props.balance)} {cc_unit}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_skynet = synt_to_colouredcoin_string(balance_ptotal);
  const balance_pending_skynet = synt_to_colouredcoin_string(balance_pending);
  const balance_change_skynet = synt_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
    balancebox_3 +
    balance_ptotal_skynet +
    balancebox_unit +
    balancebox_hline +
    balancebox_4 +
    balancebox_row +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = colouredcoin_to_synt(amount_input.value);
    const fee = colouredcoin_to_synt(fee_input.value);

    if (address.includes('skynet_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
