...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@skynet/api-react'
...
export default function Block() {
  ...
  const poolReward = syntToSkynet(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = syntToSkynet(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const skynetFees = blockRecord.fees !== undefined
    ? syntToSkynet(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.skynetexplorer.com/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.skynetexplorer.com/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: skynetFees ? `${skynetFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <>
      ...
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Skynet blockchain
            </Trans>
          </Back>
        }
        action={
