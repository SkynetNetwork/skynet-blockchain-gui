...
import skynetEnvironment from '../util/skynetEnvironment';
import skynetConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!skynetEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    skynetConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!skynetConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !skynetConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (skynetConfig.manageDaemonLifetime()) {
        skynetEnvironment.startSKYNETDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Skynet Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Skynet-Network/skynet-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Skynet-Network/skynet-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Skynet-Network/skynet-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Skynet-Network/skynet-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Skynet-Network/skynet-blockchain/issues',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Skynet Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Skynet' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Skynet Blockchain' }),
            click: () => {
              openAbout();
            },
          },
