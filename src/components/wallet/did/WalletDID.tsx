...
import { AlertDialog, Card, Flex, Loading, Dropzone } from '@skynet/core';
import {
...
import { synt_to_skynet_string } from '../../../util/skynet';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {synt_to_skynet_string(props.balance)} TXNT
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'skynet_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'skynet_addr://') {
