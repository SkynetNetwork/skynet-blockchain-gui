...
global.daemon_rpc_ws = `wss://${self_hostname}:9988`;
...
function loadConfig(net) {
  try {
    // check if SKYNET_ROOT is set. it overrides 'net'
    const config_root_dir =
      'SKYNET_ROOT' in process.env
        ? process.env.SKYNET_ROOT
        : path.join(os.homedir(), '.skynet', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 9988); // jshint ignore:line
