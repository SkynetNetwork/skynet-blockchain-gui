...
const getSKYNETVersion = () => {
  let version = null;
  const exePath = getExecutablePath('skynet');
  // first see if we can get a skynet exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or skynet is on the path
    try {
      version = child_process
